const { InfantFormDetails } = require("../../models/infant/infantFormModel");
const mongoose = require("mongoose");

const getDetails = async (req, res) => {
  const details = await InfantFormDetails.find({}).sort({ createdAt: -1 });
...
const getDetail = async (req, res) => {
  ...
  const detail = await InfantFormDetails.findById(id);
  if (!detail) {
    return res
      .status(400)
      .json({ status: false, message: "Form does not exist !" });
...
const createDetail = async (req, res) => {
  ...
  try {
    const detail = await InfantFormDetails.create({
      firstName,
      lastName,
      parentGuardianName,
      relationToAdult,
...
const deleteDetail = async (req, res) => {
  ...
  const detail = await InfantFormDetails.findOneAndDelete({ _id: id });
  if (!detail) {
    return res
      .status(400)
      .json({ status: false, message: "Detail does not exist !" });
