const { AdultFormDetails } = require("../../models/adults/adultFormModel");
const mongoose = require("mongoose");

const getDetails = async (req, res) => {
  const details = await AdultFormDetails.find({}).sort({ createdAt: -1 });
...
const getDetail = async (req, res) => {
  ...
  const detail = await AdultFormDetails.findById(id);
  if (!detail) {
    return res
      .status(400)
      .json({ status: false, message: "Form does not exist !" });
...
const createDetail = async (req, res) => {
  ...
  try {
    const detail = await AdultFormDetails.create({
      firstName,
      lastName,
      age,
      phoneNumber,
...
const deleteDetail = async (req, res) => {
  ...
  const detail = await AdultFormDetails.findOneAndDelete({ _id: id });
  if (!detail) {
    return res
      .status(400)
      .json({ status: false, message: "Detail does not exist !" });
